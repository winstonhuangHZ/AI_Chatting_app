...
enum AppLanguage: String, CaseIterable, Identifiable {
    ...
    fileprivate static let tableEn: [String: String] = [
        ...
        "streaming.on": "Streaming (stream output)",
        "timestamp.on": "Send timestamp (let the model know current time)",
        "generation.footer": "Streaming: On → token-by-token. Time-stamp: On → system prompt carries CURRENT TIME.",
    ...
    fileprivate static let tableZhHans: [String: String] = [
        ...
        "api.key": "API 密钥",
        "generation": "生成",
        "streaming.on": "流式输出（逐字显示）",
        "timestamp.on": "发送时间戳（让模型感知当前时间）",
        "generation.footer": "流式：开 → 逐字显示；时间戳：开 → 系统提示附带 CURRENT TIME。",
        "system.prompt": "系统提示",
